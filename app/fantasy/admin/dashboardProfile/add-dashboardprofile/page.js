...
const AddDshboardProfile = () => {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${api}/admin/dashboardProfile/edit`, body, { headers });
      console.log(response.data, "response from api");
     toast.success(`${response.data.message}`,{className:"text-sm"});

     setTimeout(() => {
      router.push("/fantasy/admin/dashboardProfile");
  ...
  return (
    <AddNewPageButtons pagename={pagename}  setshow={addnewpagebtn} email={email} breadscrums={breadscrums}
            ...
            <div className='flex flex-col gap-4'>
              ...
              {ButtonActive ? (
                  <div
                    ...
                    className="bg-[#1581ed] text-center cursor-pointer  border-2 border-solid border-gray-400 text-center cursor-pointer rounded-md text-white text-xs px-2 py-0.5 w-[80px] animate__animated  animate__pulse"
                  >
                   Active
                  </div>
                ) : (
        ...
        <div className="flex flex-col mt-4 w-[100%]">
          <div className="grid grid-cols-1 w-full gap-4">
            {params.map((param, index) => (
              <div key={index} className="flex items-center w-full grid-cols-2 gap-5 p-4">
                <div className="w-[45%]">
                  <Autocomplete
