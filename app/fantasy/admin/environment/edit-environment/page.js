...
const [creationTime, setcreationTime] = useState();
  ...
  const handlePostClick = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/fantasy/admin/environment");
  ...
  const handlefetchData = async (jwtToken) => {
    try {
      ...
      const response = await axios.post(`${api}/admin/environment/getedits`, body, { headers });
      ...
      const updatedData = prevDataSources.map((item, i) =>
        i === index
          ? {
            ...item,
            params: item.params.map((param, j) =>
              j === paramIndex ? { ...param, [fieldName]: !param[fieldName] } : param
            ),
          }
          : item
      );
                        ...
                        <div key={item.recordId} className="flex flex-col bg-white rounded-md ">
                            ...
                            <MultiSelectSubsidiary
                              ...
                              setSelectedSubsidiary={(newNode) => {
                                const updatedFields = editInputfields.map((field, i) =>
                                  i === index ? { ...field, subsidiaries: newNode } : field
                                );
                                seteditInputfields(updatedFields);
                              }}
                            />
                          ...
                          <div className="w-full flex flex-row p-2 justify-end items-center">
                            <div
                              ...
                              className={`border-2 rounded-md text-xs px-2 py-0.5 w-[80px] animate__animated animate__pulse ${item.status
                                ? "bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]"
                                : "bg-white text-gray-500 border-[#1581ed]"
                                }`}
                            >
                                    ...
                                    <div className="flex flex-row w-full gap-5">
                                      ...
                                      <div className="flex flex-col gap-5 w-[33%] p-2 bg-gray-100 rounded-md">
                                        <TextField
                                          ...
                                          size="small"
                                          value={param.url}
                                          onChange={(e) => handleParamChange(index, paramIndex, 'url', e.target.value)}

                                        />
                                        <div className="w-full flex flex-row justify-between items-center">
                                          <div
                                            onClick={() => handleToggle(index, paramIndex, "waitBeforeUrl")}
                                            className={`border-2 border-solid rounded-md text-xs px-2 py-0.5 w-[80px] h-[25px] ${param.waitBeforeUrl ? "bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]" : "bg-white text-gray-700 border-gray-400 text-center cursor-pointer"
                                              }`}
                                          >
                                            Before Url
                                          </div>
                                          <div
                                            onClick={() => handleToggle(index, paramIndex, "waitAfterUrl")}
                                            className={`border-2 border-solid rounded-md text-xs px-2 py-0.5 w-[70px] h-[25px] ${param.waitAfterUrl ? "bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]" : "bg-white text-gray-700 border-gray-400 text-center cursor-pointer"
                                              }`}
                                          >
                                            After Url
                                          </div>
                                      ...
                                      <div className="flex flex-col gap-5 w-[33%] p-2 bg-gray-100 rounded-md">
                                        <TextField
                                          ...
                                          onChange={(e) => handleParamChange(index, paramIndex, 'actionElement', e.target.value)}
                                        />
                                        <div className="w-full flex flex-row justify-between items-center">
                                          <div
                                            onClick={() => handleToggle(index, paramIndex, "waitBeforeClick")}
                                            className={`border-2 border-solid rounded-md text-xs px-2 py-0.5 w-[90px] h-[25px] ${param.waitBeforeClick
                                              ? "bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]"
                                          ...
                                          <div
                                            onClick={() => handleToggle(index, paramIndex, "waitAfterClick")}
                                            className={`border-2 border-solid rounded-md text-xs px-2 py-0.5 w-[80px] h-[25px] ${param.waitAfterClick
                                              ? "bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]"
                                              : "bg-white text-gray-700 border-gray-400 text-center cursor-pointer"
                                              }`}
                                          >
                                      ...
                                      <div className="w-[33%] bg-gray-100 rounded-md p-2">
                                        <TextField
                                          ...
                                          onChange={(e) => handleParamChange(index, paramIndex, 'shortDescription', e.target.value)}
                                        />
                                      </div>
                                    </div>
                                      ...
                                      <div className="flex flex-row gap-5 w-[45%]">
                                        ...
                                        <div className="flex flex-col gap-5 w-[75%]">
                                          <TextField
                                            ...
                                            onChange={(e) => handleParamChange(index, paramIndex, 'loginName', e.target.value)}
                                          />
                                          <TextField
                                            className="w-full"
                                            ...
                                            onChange={(e) => handleParamChange(index, paramIndex, 'loginNameUiSelector', e.target.value)}
                                          />
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex flex-row gap-5 w-[45%]">
                                        ...
                                        <div className="flex flex-col gap-5 w-[75%]">
                                          <TextField
                                            ...
                                            onChange={(e) => handleParamChange(index, paramIndex, 'loginPassword', e.target.value)}
                                          />
                                          <TextField
                                            className="w-full"
                                            placeholder="Enter UI selector (ID/CSS/Xpath)"
                                            ...
                                            onChange={(e) => handleParamChange(index, paramIndex, 'loginPasswordSelector', e.target.value)}
                                          />
                                        </div>
                                      </div>
                                    </div>
