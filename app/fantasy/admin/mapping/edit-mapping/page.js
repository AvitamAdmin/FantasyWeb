...
const [creationTime, setcreationTime] = useState();
  ...
  const handlefetchData = async (jwtToken) => {
    try {
      ...
      setlastmodifideBy(response.data.sourceTargetMappings[0]?.lastModified || "");
      setmodifiedBy(response.data.sourceTargetMappings[0]?.modifiedBy || "");
 setcreationTime(response.data.sourceTargetMappings[0]?.creationTime || "");
      ...
      if (response?.data?.sourceTargetMappings) {
        ...
        const dataRelationValue = response.data.sourceTargetMappings[0]?.dataRelation || "";
        const dataRelationParamValue =
          response.data.sourceTargetMappings[0]?.sourceTargetParamMappings[0]?.dataSource || "";
                              ...
                              <div className="flex flex-row gap-3 w-full justify-start">
                                ...
                                <div className="flex gap-3 mt-4">
                                  <div
                                    ...
                                    className={`${item.EnableCurrentPage
                                      ? 'bg-[#1581ed] text-center cursor-pointer  text-white border-[#1581ed]'
                                      : 'bg-[#fff] text-gray-500 border-gray-400 text-center cursor-pointer'
                                    ...
                                    param ? (
                                      <div key={paramIndex} className="items-center justify-between gap-5 grid grid-cols-5 w-[100%] p-2">
                                        <TextField
                                          ...
                                          size="small"
                                          value={param.header || ""}
                                          onChange={(e) => handleParamChange(index, paramIndex, "header", e.target.value)}
                                        />
<Autocomplete
  ...
  onChange={(event, newValue) => {
    handleParamChange(index, paramIndex, "dataSource", newValue?.recordId || "");
  ...
  renderInput={(params) => (
    <TextField {...params} label="Select Data Source" variant="standard" />
  )}
/>
...
<Autocomplete
  ...
  value={datasourceDataParams.find((opt) => String(opt) === String(param.param)) || param.param || null}
  onChange={(event, newValue) => {
    handleParamChange(index, paramIndex, "param", newValue || "");
  }}
...
/>
                                        ...
                                        <div className="flex flex-row gap-5 justify-center items-end">
                                          <div
                                            onClick={() => toggleButtonState(index, paramIndex, "isPivot")}
                                            className={`${param.isPivot ? "bg-[#1581ed] text-center cursor-pointer  text-white" : "bg-[#fff] text-gray-700 text-center cursor-pointer"} border-2 border-solid border-${param.isPivot ? "#1581ed" : "gray-400"} rounded-md text-xs px-2 py-0.5 w-[70px] h-[25px]`}
                                          >
                                            Pivot
                                          </div>
                                          <div
