...
const EditQACronJob = () => {
      ...
      if (name === "subsidiary") {
        ...
        try {
          const [
            ...
            axios.post(`${api}/qa/getEnvironmentsForSubsidiary`, { recordId: subsidiaryId }, { headers }),
            axios.post(`${api}/qa/getSitesForSubsidiary`, { recordId: subsidiaryId }, { headers }),
            axios.post(`${api}/qa/getTestPlanForSubsidiary`, { recordId: subsidiaryId }, { headers }),
            axios.post(`${api}/qa/getTestProfileForSubsidiary`, { recordId: subsidiaryId }, { headers }),
            axios.post(`${api}/qa/getCategoryForSubsidiary`, { recordId: subsidiaryId }, { headers }),
          ]);
  ...
  const handlePostClick = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        router.push("/fantasy/admin/qaCronJob");
      }, 2000);
  ...
  const handleDatasourceChangeWithAPIFetch = async (subsidiaryId, index, paramIndex, headers) => {
  ...
  value={
    subsidiary.find((subs) => subs.recordId === param.subsidiary) || null
  }
  onChange={(event, newValue) => {
    handleParamChange(index, paramIndex, "subsidiary", newValue?.recordId || "");

    // Check if `newValue` exists in `subsidiary` and send its `recordId` if true
    const existingRecord = subsidiary.find((subs) => subs.recordId === newValue?.recordId);
    if (existingRecord) {
      handleDatasourceChange(
        { target: { name: "subsidiary", value: newValue?.recordId || "" } },
        index,
        paramIndex
      );
    } else {
      // Handle the absence of a match if necessary
      console.log("Selected subsidiary does not match existing data.");
    }
  }}
  renderInput={(params) => (
    <TextField
...
<Autocomplete
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={(param.environments && Array.isArray(param.environments)
          ? param.environments.find((ds) => ds.recordId === param.environment)
          : null
        ) || null}
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "environment", newValue?.recordId || "")
  }
  renderInput={(params) => (
    <TextField {...params} label="Select Environment" variant="standard" />
  )}
/>
...
<Autocomplete
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={(param.siteIsoCodes && Array.isArray(param.siteIsoCodes)
          ? param.siteIsoCodes.find((ds) => ds.recordId === param.siteIsoCode)
          : null
        ) || null}
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "siteIsoCode", newValue?.recordId || "")
  }
  renderInput={(params) => (
...
/>
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={(param.testProfiles && Array.isArray(param.testProfiles)
          ? param.testProfiles.find((profile) => profile.recordId === param.testProfile)
          : null
        ) || null}
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "testProfile", newValue?.recordId || "")
  }
  renderInput={(params) => (
    <TextField {...params} label="Select Test Profile" variant="standard" />
  )}
/>
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={(param.testPlans && Array.isArray(param.testPlans)
          ? param.testPlans.find((plan) => plan.recordId === param.testPlan)
          : null
        ) || null}
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "testPlan", newValue?.recordId || "")
  }
  renderInput={(params) => (
    <TextField {...params} label="Select Test Plan" variant="standard" />
  )}
/>
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={(param.categoryIds && Array.isArray(param.categoryIds)
          ? param.categoryIds.find((category) => category.recordId === param.categoryId)
          : null
        ) || null}
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "categoryId", newValue?.recordId || "")
  }
  renderInput={(params) => (
...
/>
  ...
  getOptionLabel={(option) => option.identifier || ""}
  value={
    cronJobProfile.find((subs) => subs.recordId === param.cronProfileId) || null
  }
  onChange={(event, newValue) =>
    handleParamChange(index, paramIndex, "cronProfileId", newValue?.recordId || "")
  }
  renderInput={(params) => (
    <TextField {...params} label="Select Cronjob Profile" variant="standard" />
  )}
/>
      ...
      <div
        ...
        onClick={() => handleRemoveParamClick(index, paramIndex)}
