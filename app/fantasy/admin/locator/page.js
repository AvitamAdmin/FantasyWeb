...
const locator = () => {
  ...
  const fetchLocator = async () => {
    ...
    try {
      ...
      const body = { page, sizePerPage: sizePerPage === totalRecord ? totalRecord : sizePerPage,
        testLocators: fetchFilterInputs,
       };
      const response = await axios.post(`${api}/admin/locator`, body, { headers });
  
      setLocators(response.data.testLocators || []);
  ...
  const addnewroutepath = "/admin/locator/add-locator"
  const breadscrums = "Admin > locator"
  const cuurentpagemodelname = "Locator"
  const editnewroutepath = "/admin/locator/edit-locator";
  const aresuremodal = "Delete this Element";
  const aresuremodaltype = "Delete";
  const apiroutepath = "locator";
  const deleteKeyField = "testLocators";
  ...
  const fetchexportUrl = async () => {
    try {
      ...
      const response = await axios.get(`${api}/admin/locator/export`, {
        headers,
      });
