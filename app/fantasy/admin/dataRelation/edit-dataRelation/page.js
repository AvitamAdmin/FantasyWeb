...
const handleRemoveParamClick = (index, paramIndex) => {
  const newFields = [...editInputfields];
  newFields[index].dataRelationParams = newFields[index].dataRelationParams.filter(
    (_, i) => i !== paramIndex
  );
  ...
  const handleParamChange = (index, paramIndex, selectedValue) => {
    ...
    updatedFields[index].dataRelationParams[paramIndex].sourceKeyOne = selectedValue;
  ...
  const handlePostClick = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        router.push("/fantasy/admin/dataRelation");
      }, 2000);
...
/>
      ...
      <div className="flex flex-row gap-6 justify-end items-end">
        ...
        <div
          ...
          className={`${
            item.enableGenerator ? "bg-blue-500 text-white" : "bg-white text-blue-500"
    ...
    {item.dataRelationParams.map((param, paramIndex) => (
      <div key={paramIndex} className="flex items-center gap-5 p-4 w-full">
        <div className="flex flex-row w-[45%]">
          <Autocomplete
            ...
            value={
              dataSourceList.find((opt) => opt.recordId === param.dataSource) || null
            }
            onChange={(event, newValue) => {
              ...
              handleInputChange(syntheticEvent, index, paramIndex);
            }}
            renderInput={(params) => (
              <TextField {...params} label="Select DataSource" variant="standard" />
            )}
        ...
        <div className="flex flex-row w-[45%]">
          <Autocomplete
            ...
            onChange={(event, newValue) =>
              handleParamChange(index, paramIndex, newValue || "")
            }
            renderInput={(params) => (
