...
const [creationTime, setcreationTime] = useState();
  ...
  const handleFetchData = async (jwtToken) => {
    try {
      ...
      const body = { testDataSubtypes: selectedID.map((id) => ({ recordId: id })) };
  
      const response = await axios.post(`${api}/admin/testdatasubtype/getedits`, body, { headers });
 ...
 setcreationTime(testDataSubtypes[0]?.creationTime || "");
  ...
  const validateForm = () => {
    ...
    formValuesList.forEach((item, index) => {
      ...
      if (!Array.isArray(selectedSubsidiaryList[index]) || selectedSubsidiaryList[index].length === 0) {
            ...
            <div className="bg-white p-4 rounded-md shadow-md">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                ...
                setSelectedTestDataTypes={(newTestDataType) => {
                  const updatedTestDataTypes = [...selectedTestDataTypes];
              ...
              <div className="flex justify-end">
                <div
                  ...
                  } border-2 border-blue-500 rounded-md text-xs px-2 w-[80px] py-0.5`}
                >
