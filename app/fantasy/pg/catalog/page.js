...
const catalog = () => {
  ...
  const fetchCatalog = async () => {
    ...
    try {
      ...
      const body = {
        ...
        //searchValues
        catalogs: fetchFilterInputs,
      };
      const response = await axios.post(`${api}/admin/catalog`, body, { headers });
  ...
  const editnewroutepath = "/pg/catalog/edit-catalog"
  const addnewroutepath = "/pg/catalog/add-catalog"
  const breadscrums = "Admin > catalog"
  const cuurentpagemodelname = "catalog"
  const aresuremodal = "Delete this Element";
  const aresuremodaltype = "Delete";
  const apiroutepath = "catalog";
  const deleteKeyField = "catalogs";
