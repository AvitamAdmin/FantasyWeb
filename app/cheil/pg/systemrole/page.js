...
const systemrole = () => {
  ...
  const fetchSystemrole = async () => {
    ...
    try {
      ...
      const body = {
        ...
        //searchValues
        systemRoles: fetchFilterInputs,
      };
      const response = await axios.post(`${api}/admin/systemrole`, body, { headers });

      setSystemroles(response.data.systemRoles || []);
 ...
 const editnewroutepath = "/pg/systemrole/edit-systemrole"
  const addnewroutepath = "/pg/systemrole/add-systemrole"
  const breadscrums = "Admin > systemrole"
  const cuurentpagemodelname = "systemrole"
  const aresuremodal = "Delete this Element";
  const aresuremodaltype = "Delete";
  const apiroutepath = "systemrole";
  const deleteKeyField = "systemRoles";
