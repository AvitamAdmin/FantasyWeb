...
const addTestcase = () => {
  ...
  return (
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="flex flex-row gap-3 items-center">
                  {ButtonScreenshot ? (
                    ...
                    <button
                      onClick={() => setButtonScreenshot(!ButtonScreenshot)}
                      className="bg-[#fff] border-2 border-solid border-gray-400 rounded-md text-gray-500 text-xs px-2 py-0.5 w-[100px] animate__animated  animate__pulse"
                    >
                      No Screenshot
                    </button>
                  )}
                ...
                <div className="flex flex-row gap-3 items-center">
                  {ButtonEppssocheck ? (
                    <button
                      ...
                      className="bg-[#1581ed] border-2 border-solid border-[#1581ed] rounded-md text-white text-xs px-2 py-0.5 w-[100px] animate__animated  animate__pulse"
                    >
                      Epp sso check
                    </button>
                  ) : (
              ...
              {params.map((param, index) => (
                ...
                >
                  <div className="flex flex-row w-full justify-between">
                    ...
                    <Autocomplete
      className="w-full"
      options={nodeList || []} // Options come from nodeList state
      getOptionLabel={(option) => option.identifier || ""} // Display 'identifier' as label
      value={nodeList.find((ds) => ds.recordId === param.nodeList) || null} // Find matched nodeList item
      onChange={(event, newValue) => {
        handleParamChange(index, "nodeList", newValue?.recordId || "");
        handleDatasourceChange(index, newValue?.recordId || ""); // Fetch parameters when a nodeList is selected
      }}
      isOptionEqualToValue={(option, value) => option.recordId === value?.recordId} // Equality check
      onOpen={() => {
      ...
      renderInput={(params) => (
        <TextField
          ...
          InputProps={{
            ...
            endAdornment: (
              <>
                {loading ? <CircularProgress size={20} /> : null} {/* Show CircularProgress when loading */}
    ...
    />
                    ...
                    <Autocomplete
  className="w-full"
  options={param.parameter || []} // Use the fetched parameters, default to an empty array if undefined
  getOptionLabel={(option) => option } // Display the label from fetched parameters
  value={
    param?.parameter?.find((item) => item.value === param.selectedParameter) 
  } // Safely check if param.parameter exists and find the selected item
  onChange={(event, newValue) => {
    handleParamChange(index, "selectedParameter", newValue ); // Update the selected parameter in state
  }}
...
/>
                  ...
                  <div className="flex flex-row w-full justify-between">
                    ...
                    <div className="flex flex-row gap-5 justify-center items-center">
                      <button
                        onClick={() => toggleButtonStateparam(index, "isOrChain")}
                        className={`${
                          param.isOrChain
                        ...
                        } rounded-md text-xs px-2 py-0.5 w-[80px] h-[30px]`}
                      >
                        isOrChain
                      </button>
                      <button
                    ...
                    <div className="flex flex-row gap-3 w-full justify-between">
                      <div className="flex flex-row gap-5">
                        <button
                          ...
                          } rounded-md text-xs px-2 py-0.5 w-[120px] h-[30px]`}
                        >
                          Enter Key
                        </button>
                        <button
