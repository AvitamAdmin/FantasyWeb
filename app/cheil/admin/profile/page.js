...
import { resetDeleteStatus } from "@/app/src/Redux/Slice/slice";
...
const profile = () => {
  ...
  const fetchProfiles = async () => {
    ...
    try {
      ...
      const body = { page, sizePerPage: sizePerPage === totalRecord ? totalRecord : sizePerPage,
        testProfiles: fetchFilterInputs,
       };
      const response = await axios.post(`${api}/admin/profile`, body, { headers });
  
      setProfiles(response.data.testProfiles || []);
  ...
  const addnewroutepath = "/admin/profile/add-profile"
  const breadscrums = "Admin > profile"
  const cuurentpagemodelname = "Profile"
  const editnewroutepath = "/admin/profile/edit-profile";
  const aresuremodal = "Delete this Element";
  const aresuremodaltype = "Delete";
  const apiroutepath = "profile";
  const deleteKeyField = "testProfiles";
